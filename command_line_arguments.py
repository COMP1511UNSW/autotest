...
def parse_arguments():
	...
	parser.add_argument("-a", "--autotest_directory", help="DIRECTORY containing test specification")
	...
	parser.add_argument("-E", "--exercise_directory", action="append", help="parent DIRECTORY containing exercises")
	parser.add_argument("-f", "--file", nargs='+', default=[], help="add a copy of this file to the test directory ")
	parser.add_argument("-g", "--generate_expected_output", nargs='?', const='stdout', default="no", help="generate expected output for tests based on supplied solution")
	...
	parser.add_argument("-P", "--parameters", help="set parameter values", action="append")

	parser.add_argument("extra_arguments",  nargs='*', default=[], help="")

	source_args = parser.add_mutually_exclusive_group()
	source_args.add_argument("-D", "--directory", help="add files from this directory to the test directory")
...
def normalize_arguments(parser, args, tests):
	...
	for arg in args.extra_arguments:
		...
		if any(fnmatch.fnmatch(arg, f) for f in files):
			...
			if not matching_labels:
				if not os.path.exists(os.path.join(args.autotest_directory, 'runtests.pl')):
	...
	if args.debug: print('programs:', args.programs, file=sys.stderr)
	...
	if args.debug: print('labels:', args.labels, file=sys.stderr)
	...
	if args.debug: print('normalized args:', args, file=sys.stderr)
...
def find_test_specification(args):
	...
	if args.autotest_directory:
		...
		if args.marking:
			sub_pathnames = ['automarking,txt', 'automarking/tests.txt', 'automarking/automarking.txt']
		else:
			sub_pathnames = ['tests.txt', 'autotest/tests.txt', 'autotest/automarking.txt']

	test_specification_pathname = find_autotest_dir(exercise_directories, exercise, sub_pathnames, debug=args.debug)
...
#	parser.add_argument("-j", "--json", help=argparse.SUPPRESS)
	...
	parser.add_argument("--no_show_expected", dest="show_expected", help=argparse.SUPPRESS)
	...
	parser.add_argument("--no_show_reproduce_command", dest="show_reproduce_command", help=argparse.SUPPRESS)
	parser.add_argument("--no_check_hash_bang_line", dest="check_hash_bang_line", help=argparse.SUPPRESS)
	parser.add_argument("--no_fail_tests_for_errors", dest="fail_tests_for_errors", help=argparse.SUPPRESS)
	parser.add_argument("--show_stdout_if_errors", dest="show_stdout_if_errors", help=argparse.SUPPRESS)
