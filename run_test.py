...
class Test():
	def __init__(self, autotest_dir, **parameters):
		...
		if parameters.get('compare_only_characters', ''):
			...
			if debug: print('compare_only_characters', parameters['compare_only_characters'])
...
#		mapping['\r'] = '\n'
	...
	def run_test(self, compile_command=''):
		...
		if not self.test_passed:
			self.failed_compiler = " ".join(compile_command) if isinstance(compile_command, list) else str(compile_command)
	...
	def check_files(self):
		for (pathname, expected_contents) in self.parameters['expected_files'].items():
			try:
				...
				actual_contents = ''
			short_explanation = self.check_stream(actual_contents, expected_contents, f"file: {pathname}")
	...
	def make_string_canonical(self, raw_str, keep_all_lines=False):
		...
		if filter:
			if self.debug: print("postprocess_output_command=%s str='%s'" % (filter, s))
			p = subprocess.run(filter, stdout=subprocess.PIPE, input=s, stderr=subprocess.PIPE, shell=isinstance(filter, str), universal_newlines=True)
			if p.stderr:
				raise InternalError('error from postprocess_output_command: ' + p.stderr)
			if p.returncode:
				raise InternalError('non-zero exit status from postprocess_output_command')
			s = p.stdout
			if self.debug: print("after filter s='%s'"%s)
	...
	def get_long_explanation(self):
		...
		colored = termcolor_colored if self.parameters['colorize_output'] else lambda x,*a,**kw: x
#		colored = lambda x,*a,**kw: x # disable use of blue below - hard to read, replace with more readable color
		...
		if not self.stderr_ok:
			if self.expected_stderr:
				self.long_explanation += self.report_difference("stderr", self.expected_stderr, self.stderr)
				...
				self.long_explanation += colored(sanitize_string(self.stdout, **self.parameters), 'cyan')
				...
				if 'Error too much output' in self.stderr:
					...
					errors += colored(sanitize_string(self.stdout, **self.parameters), 'yellow')
		...
		if self.parameters['show_reproduce_command']:
			...
			if self.failed_compiler:
				self.long_explanation += colored(indent + self.failed_compiler + "\n", 'blue')
	...
	def report_difference(self, name, expected, actual):
		...
		return explain_output_differences(name, expected, canonical_expected, canonical_expected_plus_newlines, actual, canonical_actual, canonical_actual_plus_newlines, **self.parameters)
