...
class Parameter:
	def __init__(self, name, default=None, description='', required_type=None, finalize=None, must_be_set=None, show_in_documentation=True):
		self.name = name
...
def finalize_command(parameter_name, value, parameters):
	if isinstance(value, list):
		...
		return []
	raise TestSpecificationError(f"invalid value for parameter '{parameter_name}': {value}")					
...
PARAMETER_LIST += [
	...
	Parameter(
		"files",
		default = lambda parameters: [parameters['program'] + '.c'] if '.' not in parameters['program'] else [parameters['program']],
...
PARAMETER_LIST += [
	...
	Parameter(
		...
		default = lambda p: dict((k,v) for (k,v) in os.environ.items() if re.fullmatch(p['environment_kept'], k)),
		show_in_documentation=False,
		description = """
...
PARAMETER_LIST += [
	...
	Parameter(
		...
		default = lambda p: {
			...
			'PERL5LIB' : '.',
			'HOME' : '.',
			'PATH' : '/bin:/usr/bin:/usr/local/bin:.:' + p['__environment_original'].get('PATH', ''),
			},
...
def finalize_expected_files(name, expected_files, parameters):
	...
	if not isinstance(expected_files, dict):
		raise TestSpecificationError(f"error invalid value for parameter '{name}': {expected_files}")
	...
	for (k,v) in list(expected_files.items()):
		if not isinstance(k, str):
			raise TestSpecificationError(f"error invalid type for parameter '{name}' key: {k}")
...
PARAMETER_LIST += [
	...
	Parameter(
		"ignore_characters",
		default = '',
		finalize = lambda name, value, parameters: \
			''.join(set(value + string.whitespace if parameters['ignore_whitespace'] else value) - set("\n")),
		description = """
...
def finalize_dcc_output_checking(name, value, parameters):
	...
	if (value is None or value == '' or str(value).lower()[0:1] == 'a') and (
			len(parameters['files']) != 1 or
			not parameters['files'][0].endswith('.c') or
			parameters['expected_stderr'] or
			parameters.get('postprocess_output_command', '') or
			parameters.get('compiler_args', '') or
			(isinstance(parameters['command'], str) and (set(';|&') & set(parameters['command'])))
		):
		return False

	for p in "expected_stdout ignore_case compare_only_characters ignore_characters ignore_trailing_whitespace ignore_whitespace ignore_blank_lines max_stdout_bytes".split():
...
PARAMETERS = collections.OrderedDict((bv.name, bv) for bv in PARAMETER_LIST if isinstance(bv, Parameter))
...
def normalize_parameters(parameters, check_required_parameters_set=True, debug=0):
	...
	"""
	error_prefix = f"{parameters.get('_source_name', '?')}:{parameters.get('_line_number', '?')}"
	try:
...
def set_parameter_aliases(parameters, debug=0):
	...
	for (parameter,value) in list(parameters.items()):
		...
		if alias:
			if alias[0] == '?':	
				continue
				raise TestSpecificationError(f"error - not yet implemented parameter '{parameter}'")
...
def set_parameter_constant_defaults(parameters, debug=0):
	for p in PARAMETERS.values():
		if (
			p.name not in parameters and
			p.default is not None and
		...
		if p.name in parameters:
			...
			if p.required_type is not None and not isinstance(value, p.required_type):
				try:
					...
					raise TestSpecificationError(f"error invalid value for parameter '{p.name}': {value}")
...
def check_valid_parameter_name(name):
	return (
		...
		name in PARAMETER_ALIASES or
		name in NEGATED_PARAMETER_ALIASES or
