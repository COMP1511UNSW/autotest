...
def fetch_submission(temp_dir, args):
	...
	if args.tarfile:
		...
		if re.search(r'^https?://.*\.tar(.[a-z]+)?', args.tarfile):
			...
			execute(['tar', '-x', '-C', temp_dir, '-f', args.tarfile], print_command=args.debug)
	elif args.directory:
		...
		if os.path.isdir(".git") and os.path.isdir(args.exercise) and os.listdir(args.exercise) and os.path.realpath(args.autotest_directory) != os.path.realpath(args.exercise):
			print('cd', args.exercise)
			os.chdir(args.exercise)
